...
public class Grafo <E extends Comparable<E>> {
    ...
    public LinkedList<Vertice<E>> getVertice() {
        return vertices;
    }

    public void setVertice(LinkedList<Vertice<E>> vertices) {
        this.vertices = vertices;
    }
    ...
    public Stack<Vertice<E>> menorCamino(Vertice<E> inicio, Vertice<E> destino) throws ExceptionVertice{
    	Stack<Vertice<E>> verticesCamino = new Stack<Vertice<E>>();
    	if(destino.getAnterior() == null) {
    		throw new ExceptionVertice("No existe camino");
    	}
    	while (destino!=null){
            verticesCamino.push(destino);
    ...
    public Stack<Vertice<E>> menorCaminoSinPesos(Vertice<E> inicio, Vertice<E> destino) throws ExceptionVertice{
    	   ...
    	   Queue<Vertice<E>> verticesPendientes=new LinkedList<Vertice<E>>();
    	   inicio.setDistance(0);
    	   verticesPendientes.add(inicio);
    	   Iterator<Arista<E>> it;
    	   
    	   while(!verticesPendientes.isEmpty()) {
    		   Vertice<E> actual=verticesPendientes.poll(); 
    		   it=actual.getAdyacentes().iterator();
    		   
    		   while(it.hasNext()) {
    			   Vertice<E> aux=it.next().getDestino();
    			   if(aux.getAnterior() == null && aux != inicio) {
    				   aux.setAnterior(actual);
    				   aux.setDistance(actual.getDistance()+1);
    				   verticesPendientes.add(aux);
    			   }
    		   }
    	   }
    	   return menorCamino(inicio,destino);
    }
    
    public Stack<Vertice<E>> dijkstra(Vertice<E> inicio, Vertice<E> destino) throws ExceptionVertice {
 	   inicializarVertices();
 	   
 	   PriorityQueue<Vertice<E>> verticesPendientes=new PriorityQueue<Vertice<E>>();
 	   inicio.setDistance(0);
 	   verticesPendientes.add(inicio);
 	   Iterator<Arista<E>> it;
 	   ...
 	   while(!verticesPendientes.isEmpty()) {
 		   ...
 		   while(it.hasNext()) {
 			   Arista<E> aristaAux=it.next();
 			   Vertice<E> aux=aristaAux.getDestino();
 			   
 			   if((aux.getAnterior() == null && aux != inicio) || (aux.getAnterior() != null  && actual.getDistance()+ aristaAux.getPeso() <aux.getDistance() )) {
 				   aux.setAnterior(actual);
 				   aux.setDistance(actual.getDistance()+aristaAux.getPeso());
 				   verticesPendientes.add(aux);
 			   }
 		   }
 	   }
    ...
    public static void main(String[] args) {
        ...
        grafo.getVertice().add(v1);
        grafo.getVertice().add(v2);
        grafo.getVertice().add(v3);
        grafo.getVertice().add(v4);
        grafo.getVertice().add(v5);
        grafo.getVertice().add(v6);
        grafo.getVertice().add(v7);
